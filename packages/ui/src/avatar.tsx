...
export function getUserAvatarUrl(user?: User | null) {
  ...
  if (!user?.id) return "https://api.thereflist.com/og/avatar";

  const ogAvatar = `https://api.thereflist.com/og/avatar?seed=${encodeURIComponent(user.id)}`;
  const encodedOGAvatar = `https://api.thereflist.com/og/avatar${encodeURIComponent(`?seed=${encodeURIComponent(user.id)}`)}`;
...
export function Avatar({
  ...
  return (
    <img
      ...
      draggable={false}
      onError={() => {
        setUrl(`https://api.thereflist.com/og/avatar?seed=${user.id}`);
      }}
    />
  );
}
