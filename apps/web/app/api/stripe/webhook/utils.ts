  ...
  return await Promise.all(
    owners.map(
      (owner) =>
        ...
        sendEmail({
          email: owner.email,
          from: "Steven Tey <steven@thereflist.com>",
          replyTo: "steven.tey@thereflist.com",
          subject: "Feedback for RefList?",
          text: `Hey ${owner.name ? owner.name.split(" ")[0] : "there"}!\n\nSaw you canceled your RefList subscription${reasonText ? ` and mentioned that ${reasonText}` : ""} – do you mind sharing if there's anything we could've done better on our side?\n\nWe're always looking to improve our product offering so any feedback would be greatly appreciated!\n\nThank you so much in advance!\n\nBest,\nCarra Wu\nFounder, thereflist.com`,
        }),
    ),
  );
}
